...
Usage from app.py:
    from orchestrator import run_pipeline, run_conversational_turn
...
from EmailSenderAgent import generate_and_send_email, format_email_preview  # ← Agent 3 (send)
...
def _name_part_in_message(part: str, msg_lower: str) -> bool:
    """
    Word-boundary check for a name fragment inside a message.
    Plain substring containment is too loose: a candidate's own email address
    (e.g. maryemahmed707@gmail.com) can contain her name as a substring,
    causing false "name mentioned" matches when the user just pastes an email.
    """
    return re.search(rf"\b{re.escape(part)}\b", msg_lower) is not None
...
def _find_named_candidate(message: str, candidates: list) -> list:
    ...
    for c in candidates:
        ...
        if any(_name_part_in_message(part, msg_lower) for part in all_name_parts):
            matched.append(c)
...
# a compare/filter request, a pagination request, an email request,
# or a brand-new search query — so app.py doesn't have to guess.
# ══════════════════════════════════════════════════════════════════════════════
...
_COMPARE_TRIGGERS = [
    ...
    # pronoun + experience/skill phrasing — e.g. "does she have experience with RAG",
    # "does he know Docker", "do they have X" — these are filter questions over
    # the existing pool, not a request for brand-new candidates.
    "does she have", "does he have", "do they have",
    "does she know", "does he know", "do they know",
    "is she experienced", "is he experienced",
    "has she", "has he", "have they",
    # Arabic
    "مين أحسن", "مين أفضل", "مين عنده", "مين بنى", "مين شتغل",
    "قارن", "ايه الفرق", "مين منهم",
]

# Phrases that signal "send an email to a candidate" — Agent 3
_EMAIL_TRIGGERS = [
    "send email", "send an email", "email to", "mail to",
    "interview invitation", "send to", "email them", "email the candidate",
]
...
def _is_reasoning_request(message: str, last_candidates: list) -> bool:
    ...
    2. Refers to the candidate(s) with a pronoun, when the result set is small
       enough that the reference is unambiguous.
    3. Mentions a retrieved candidate's name.
    ...
    # Pronoun follow-up — only safe to assume "about the candidate(s)" when
    # there's a small, unambiguous set already on screen.
    if len(last_candidates) <= 3:
        words = set(re.findall(r"\b\w+\b", msg_lower))
        if words & set(_PRONOUN_TRIGGERS):
            return True
    ...
    for c in last_candidates:
        ...
        if any(_name_part_in_message(p, msg_lower) for p in name_parts):
            return True
...
def _is_compare_request(message: str, last_candidates: list) -> bool:
    ...
    if not last_candidates:
        return False
    msg_lower = message.lower().strip()
    if any(trigger in msg_lower for trigger in _COMPARE_TRIGGERS):
        return True

    # Regex fallback: "who/which ... has/have/know/built/used/did/worked"
    # with arbitrary words in between — literal trigger phrases require the
    # words to be adjacent, which misses real phrasing like
    # "who of them has experience with X" or "which of these knows Docker".
    if re.search(
        r"\b(who|which)\b.{0,20}\b(has|have|know|knows|built|used|did|worked)\b",
        msg_lower,
    ):
        return True
...
def _is_email_request(message: str) -> bool:
    """
    Return True if the message is asking to send an email to a candidate.
    Checked BEFORE reasoning/compare/search so "send an email to ..." never
    gets misread as a new CV search.
    """
    msg_lower = message.lower().strip()
    if any(trigger in msg_lower for trigger in _EMAIL_TRIGGERS):
        return True

    # Catch phrasing the literal trigger list misses, e.g. "send her email",
    # "send him an email" — a send-verb plus the word "email"/"mail"
    # anywhere in the message is a strong enough signal on its own.
    has_send_verb = "send" in msg_lower
    has_mail_noun = "email" in msg_lower or "mail" in msg_lower
    return has_send_verb and has_mail_noun


def _extract_email_from_message(message: str) -> str | None:
    """Extract the first email address found in the message, if any."""
...
def _send_email_reply(user_message: str, last_candidates: list) -> tuple[str, str]:
    """
    Handle an email-send request using Agent 3 (EmailSenderAgent).
    Returns (reply_html, status_text).
    """
    ...
    if to_email and last_candidates:
        for c in last_candidates:
            if (c.get("metadata", {}).get("email", "") or "").lower() == to_email.lower():
                candidate = c
                break
    ...
    if candidate:
        ...
        # No matching candidate in memory — try sending using just the email
        # address found in the message (generate_and_send_email extracts it itself).
    ...
    if status.startswith("✅"):
        ...
        reply_html = (
            ...
            "border-radius:6px;margin-bottom:12px'>✅ Email sent successfully!</div>"
            f"{preview}</div>"
        )
    else:
        reply_html = (
            ...
            f"border-radius:6px;'>{status if status.strip().startswith('❌') else '❌ ' + status}</div></div>"
        )

    return reply_html, status
...
# Handles five cases, checked in this order:
#   0. Send-email request   → Agent 3 (EmailSenderAgent) sends a real email
#   1. "Why" follow-up      → Agent 2 reasoning over stored candidates
#   2. Compare / filter     → answered from cached candidates, no new search
#   3. "More results"       → pagination over the same underlying query
#   4. Anything else        → brand-new search (Agent 1)
# ══════════════════════════════════════════════════════════════════════════════
...
def run_conversational_turn(
    ...
    # ── Case 0: Send-email request ───────────────────────────────────────────
    # Checked FIRST so "send an email to john@x.com" never gets misread as a
    # brand-new CV search.
    if _is_email_request(user_message):
        print(f"[Orchestrator] Email request detected")

        reply_html, status = _send_email_reply(user_message, last_candidates or [])
