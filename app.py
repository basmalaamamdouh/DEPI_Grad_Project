...
from orchestrator import run_conversational_turn
...
QUALITY_STYLE = {
    "strong":  ("#059669", "STRONG MATCH"),
    "good":    ("#D97706", "GOOD MATCH"),
    "partial": ("#9CA3AF", "PARTIAL"),
}
...
# ══════════════════════════════════════════════════════════════════════════════
# THEME  — Clean enterprise: Inter + neutral greys, indigo accent
# ══════════════════════════════════════════════════════════════════════════════
...
THEME = gr.themes.Default(
    ...
    # Accent — indigo (professional, neutral-industry)
    color_accent="#4F46E5",
    ...
    button_cancel_background_fill_dark="#FEF2F2",
...
:root {
  ...
  --ink:#111827; --ink-2:#374151; --ink-soft:#6B7280; --line:#E5E7EB;
...
/* ── Base — full width, no max-width cap ── */
.gradio-container {
...
/* Gradio wraps everything in an extra .wrap div — blow it open too */
.gradio-container > .wrap,
.gradio-container > div > .wrap { max-width:100% !important; }

/* ── Header ── */
#header-strip {
  background:var(--surface); border:1px solid var(--line);
  border-radius:var(--radius); padding:28px 36px 26px;
...
#title-text  { font-weight:700; font-size:1.55em; color:var(--ink); margin:0; }
#subtitle-text { font-size:.88em; color:var(--ink-soft); margin:6px 0 0; line-height:1.6; }

/* ── Tabs — clean pill nav ── */
.tabs > .tab-nav {
  border-bottom:1px solid var(--line) !important;
  padding-bottom:0 !important; gap:4px;
}
.tabs > .tab-nav button, button[role="tab"] {
  ...
  color:var(--ink-soft) !important; border:none !important;
  background:transparent !important;
  transition:color .15s, background .15s !important;
}
.tabs > .tab-nav button:hover:not(.selected), button[role="tab"]:hover:not([aria-selected="true"]) {
...
/* ── Search button — contained, not full-width banner ── */
#search-btn-wrap {
...
/* Override Gradio's full-width default for primary buttons */
.gradio-container button.primary { width:auto !important; }

/* ── Sliders / controls row spacing ── */
.gradio-container .block { border-radius:var(--radius-sm) !important; }
.gradio-container label span { font-size:.8em !important; font-weight:500 !important; color:var(--ink-soft) !important; }

/* ── Utility ── */
.log-box textarea { font-family:'JetBrains Mono',monospace !important; font-size:.82em !important; }
#results-html { max-height:800px; overflow-y:auto; padding-right:4px; }

/* ── Status / rewrite / empty ── */
.status-line  { font-size:.84em; color:var(--ink-soft); font-weight:500; }
.rewrite-note {
  font-size:.82em; color:var(--ink-soft); margin-bottom:12px;
...
.empty-state {
  text-align:center; padding:72px 20px; color:var(--muted); font-size:.93em;
}

/* ── Candidate card ── */
.frame-card {
  background:var(--surface); border:1px solid var(--line);
  border-radius:var(--radius); padding:20px 24px 18px;
  margin:0 0 14px; box-shadow:var(--shadow);
  transition:box-shadow .15s, border-color .15s;
}
.frame-card:hover { box-shadow:0 4px 16px rgba(0,0,0,.09); border-color:#d1d5db; }

/* Remove old cyberpunk corner notches */
.frame-card::before, .frame-card::after,
.frame-card .corner-bl, .frame-card .corner-br { display:none !important; }

.frame-head  { display:flex; align-items:flex-start; gap:16px; }
.frame-index {
  font-family:'JetBrains Mono',monospace; font-size:.68em;
  color:var(--muted); letter-spacing:.06em; display:block; margin-bottom:3px;
}
.frame-name  { font-weight:600; font-size:1.05em; color:var(--ink); }
.frame-file  { font-family:'JetBrains Mono',monospace; font-size:.7em; color:var(--ink-soft); margin-top:2px; }

.gauge-col   { display:flex; flex-direction:column; align-items:center; margin-left:auto; flex-shrink:0; }
.gauge-pct   { font-weight:700; font-size:1.25em; margin-top:-12px; }
.gauge-label { font-size:.65em; letter-spacing:.08em; color:var(--muted); text-transform:uppercase; margin-top:2px; }
.quality-tag {
  font-size:.68em; letter-spacing:.06em; text-transform:uppercase;
  font-weight:600; display:flex; align-items:center; gap:5px; margin-top:6px;
}
...
.contact-label { font-size:.65em; letter-spacing:.06em; text-transform:uppercase; color:var(--muted); margin-bottom:3px; font-weight:500; }
.contact-value { font-size:.86em; color:var(--ink-2); }
.contact-value a { color:var(--accent); text-decoration:none; }
.contact-value a:hover { text-decoration:underline; }

/* ── Chips ── */
.chip { font-size:.7em; font-weight:500; padding:2px 9px; border-radius:999px; margin-right:5px; display:inline-block; margin-top:6px; }
...
.scan-log {
  ...
  font-size:.74em; color:var(--ink-soft); line-height:1.65;
}

/* ── Agent chat tab ── */
#agent-chatbot { border:1px solid var(--line) !important; border-radius:var(--radius) !important; }
.chat-input-row textarea { font-family:'Inter',sans-serif !important; }
.agent-thinking { font-size:.82em; color:var(--muted); padding:10px 0; animation:blink 1s step-end infinite; }
@keyframes blink { 0%,100%{opacity:1} 50%{opacity:.35} }
.chat-tip {
  font-size:.76em; color:var(--ink-soft); padding:5px 12px;
  border:1px solid var(--line); border-radius:999px;
  cursor:pointer; display:inline-block; margin:4px 4px 0 0;
  transition:border-color .15s, color .15s;
}
.chat-tip:hover { border-color:var(--accent); color:var(--accent); }
"""
...
with gr.Blocks(title="HR Assistant — CV RAG") as demo:
    ...
    with gr.Tabs():
        ...
        with gr.Tab("🤖 AI Assistant"):
            ...
            llm_hist        = gr.State([])
            # ↓ NEW: stores the last search's candidate dicts so "why?" turns work
            last_candidates = gr.State([])
            ...
            def on_send(user_msg: str, history: list, llm_history: list, candidates: list):
                if not user_msg.strip():
                    yield history, llm_history, "", candidates
                    return
                ...
                # ── Use the orchestrator — handles both search and "why" turns ──
                reply_html, updated_llm_history, updated_candidates = run_conversational_turn(
                ...
                yield history, updated_llm_history, "", updated_candidates

            send_btn.click(
                on_send,
                inputs=[chat_input, chatbot, llm_hist, last_candidates],
                outputs=[chatbot, llm_hist, chat_input, last_candidates],
            )
            chat_input.submit(
                on_send,
                inputs=[chat_input, chatbot, llm_hist, last_candidates],
