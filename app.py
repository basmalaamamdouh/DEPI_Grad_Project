...
# ══════════════════════════════════════════════════════════════════════════════
# ══════════════════════════════════════════════════════════════════════════════
# GAP AGENT — CALLBACK  (Agent 4)
# ══════════════════════════════════════════════════════════════════════════════

def cb_gap_report(candidate_index, jd_text: str) -> str:
    if not _last_results:
        return "<div class='empty-state'>Run a search first to load candidates.</div>"
    if not jd_text.strip():
        return "<div class='empty-state'>Please enter a job description above.</div>"
    idx = int(candidate_index)
    if idx < 0 or idx >= len(_last_results):
        return "<div class='empty-state'>Invalid candidate index.</div>"
    r    = _last_results[idx]
    m    = r["metadata"]
    name = m.get("name") or Path(m.get("file", "Candidate")).stem
    cv_text = " ".join(r.get("all_chunks", [r["text"]]))
    try:
        ...
        return (
            f"<div style='color:#C0392B;padding:12px;font-family:monospace'>"
            f"❌ Gap report failed: {exc}</div>"
        )

def cb_load_gap_candidates():
    if not _last_results:
        return gr.update(choices=[], value=None)
    choices = []
...
# ── Tab 4: Gap Report ─────────────────────────────────────────────────
        with gr.Tab("🔎 Gap Report"):
            gr.HTML(
                "<div style='font-family:JetBrains Mono,monospace;font-size:.75em;"
                "color:#5C6259;padding:8px 0 4px;letter-spacing:.04em'>"
                "AGENT 4 — CANDIDATE GAP ANALYZER</div>"
                "<p style='font-size:.92em;color:#1C1F1B;margin:0 0 14px'>"
                "Run a search first, then select a candidate and paste the job "
                "requirements to get a full gap report: missing skills, experience "
                "gaps, recommended resources, and an upskilling timeline.</p>"
            )
            with gr.Row():
                with gr.Column(scale=3):
                    gap_jd_box = gr.Textbox(
                        label="Job Description / Requirements",
                        placeholder=(
                            "Paste the full job description here, or describe what "
                            "you need — e.g. 'Senior Python developer with FastAPI, "
                            "PostgreSQL, 3+ years, fintech background'"
                        ),
                        lines=8,
                    )
                with gr.Column(scale=2):
                    gap_load_btn = gr.Button(
                        "↻  Load candidates from last search",
                        variant="secondary", size="sm",
                    )
                    gap_candidate_dd = gr.Dropdown(
                        label="Select candidate", choices=[], interactive=True,
                    )
                    gap_run_btn = gr.Button(
                        "Generate Gap Report  →", variant="primary", size="lg",
                    )
            gap_output = gr.HTML(
                "<div class='empty-state'>Select a candidate and enter the job "
                "requirements above, then click Generate Gap Report.</div>"
            )
            gap_load_btn.click(cb_load_gap_candidates, outputs=gap_candidate_dd)
            gap_run_btn.click(
                cb_gap_report,
                inputs=[gap_candidate_dd, gap_jd_box],
                outputs=gap_output,
            )
        # ── Tab 4: Setup guide ────────────────────────────────────────────────
        with gr.Tab("⚙️ Setup"):
